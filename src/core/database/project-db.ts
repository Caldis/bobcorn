/**
 * ProjectDb — typed wrapper around a sql.js Database (.icp SQLite format).
 *
 * Environment-agnostic AND renderer-safe: no Node builtins, no browser
 * globals, no file I/O. Loading sql.js and reading/writing .icp files live in
 * ./lifecycle (openProject/createEmptyProject/saveProject) so this module can
 * be imported from the renderer without pulling in `require('sql.js/...')`.
 * Guarded by test/unit/core-boundary-guard.test.js (no Node builtin imports).
 */
...
export class ProjectDb {
  ...
  initSchema(projectName: string = 'iconfont', displayName?: string): void {
    ...
    // description = project notes; projectColor = avatar color override.
    // Column order mirrors renderer initNewProject() exactly.
    this.db.run(
      `CREATE TABLE ${TABLE_PROJECT} (id varchar(255), projectName varchar(255), displayName varchar(255), description TEXT, projectColor varchar(32), createTime datetime DEFAULT CURRENT_TIMESTAMP, updateTime datetime DEFAULT CURRENT_TIMESTAMP)`
    );
    this.db.run(
  ...
  private migrateProjectColumns(): void {
    try {
      ...
      // Project notes (mirrors renderer migration exactly)
      if (!colNames.includes('description')) {
        this.db.run(`ALTER TABLE ${TABLE_PROJECT} ADD COLUMN description TEXT`);
      }
      // Avatar color override (mirrors renderer migration exactly)
      if (!colNames.includes('projectColor')) {
        this.db.run(`ALTER TABLE ${TABLE_PROJECT} ADD COLUMN projectColor varchar(32)`);
      }
    } catch (_) {
  ...
  /** Project description text (user-facing notes). */
  getProjectDescription(): string | null {
    return (this.getProjectAttributes().description as string | undefined) || null;
  }

  setProjectDescription(description: string | null): void {
    if (description) {
      this.db.run(
        `UPDATE ${TABLE_PROJECT} SET description = ${sf(description)} WHERE id = 'projectAttributes'`
      );
    } else {
      this.db.run(`UPDATE ${TABLE_PROJECT} SET description = NULL WHERE id = 'projectAttributes'`);
    }
  }

  /** Project avatar color override (hex string, e.g. "#FF8800"). */
  getProjectColor(): string | null {
    return (this.getProjectAttributes().projectColor as string | undefined) || null;
  }

  setProjectColor(color: string | null): void {
    if (color) {
      this.db.run(
        `UPDATE ${TABLE_PROJECT} SET projectColor = ${sf(color)} WHERE id = 'projectAttributes'`
      );
    } else {
      this.db.run(`UPDATE ${TABLE_PROJECT} SET projectColor = NULL WHERE id = 'projectAttributes'`);
    }
  }
  ...
  /**
   * Soft-delete an icon (single-id convenience wrapper over softDeleteIcons).
   */
  deleteIcon(id: string): void {
    this.softDeleteIcons([id]);
  }

  /**
   * Soft-delete icons: move them to the 'resource-deleted' group.
   * Variants are cascade-deleted (hard delete) — they never outlive the parent.
   */
  softDeleteIcons(ids: string[]): void {
  ...
  /**
   * Permanently delete icons AND their variants (hard delete, irreversible).
   */
  permanentDeleteIcons(ids: string[]): void {
    if (ids.length === 0) return;
    const inClause = ids.map((id) => sf(id)).join(',');
    this.db.run(
      `DELETE FROM ${TABLE_ICON} WHERE id IN (${inClause}) OR variantOf IN (${inClause})`
    );
  }

  /**
   * Move icons AND their variants into the recycle bin ('resource-recycleBin').
   * Unlike softDeleteIcons, variants are kept (recoverable together with parent).
   */
  recycleIcons(ids: string[]): void {
    if (ids.length === 0) return;
    const inClause = ids.map((id) => sf(id)).join(',');
    this.db.run(
      `UPDATE ${TABLE_ICON} SET iconGroup = 'resource-recycleBin' WHERE id IN (${inClause}) OR variantOf IN (${inClause})`
    );
  }
