---
version: alpha
name: Bobcorn-website-design
description: The design specification for the Bobcorn website (docs/ — bobcorn.caldis.me). Adopted from getdesign.md's Vercel design analysis — a stark black-and-ink duet on near-white canvas, broken at hero scale by a multi-color mesh gradient (cyan / blue / magenta / amber) that acts as the entire decorative system, paired with a geometric sans for headlines and a monospaced caption face for technical labels. All website pages MUST follow this spec; implementation mapping is in the "Bobcorn Site Implementation" section at the end.
...
## Bobcorn Site Implementation

This section maps the spec above onto the Bobcorn website source. The spec is the authority; this section states where each part lives.

### File layout

| File | Role |
|---|---|
| `docs/DESIGN.md` | This spec. The single source of truth for site visual decisions. |
| `docs/assets/site.css` | All tokens as CSS custom properties plus the shared component classes. Every page links it. |
| `docs/wiki/shared/wiki.css` | Wiki layout (sidebar / topbar / article chrome). Imports `../../assets/site.css` for tokens; defines only wiki-specific layout on top. |
| Per-page `<style>` blocks | Page-specific layout only. Every value must reference a `var(--*)` token or a shared class from `site.css`. No raw hex colors, no raw font stacks, no raw px radii/shadows. |

### Token naming

CSS custom properties in `site.css` mirror the spec token names one-to-one: `colors.canvas-soft` → `--canvas-soft`, `spacing.lg` → `--space-lg`, `rounded.pill` → `--rounded-pill`, elevation levels → `--shadow-l1` … `--shadow-l5` (+ `--shadow-ring` for the inset hairline). Typography tokens are classes: `typography.display-xl` → `.t-display-xl`, `typography.caption-mono` → `.t-caption-mono`, and the section-eyebrow pattern → `.eyebrow`.

### Fonts

Three faces load from Google Fonts via `@import` at the top of `site.css` (wiki pages also carry a direct `<link>` with the same URL):

- Geist (400 / 500 / 600) — body, buttons, UI labels (`--font-sans`)
- Space Grotesk (500 / 600) — display headlines, hero type, and the brand wordmark (`--font-display`). This is the spec's sanctioned third face ("condensed display sans for occasional editorial moments"), promoted to the display role per user review — Geist-everywhere read as too plain
- Geist Mono (400) — code, eyebrows, technical labels (`--font-mono`)

Space Grotesk tracking is softer than the Geist values in the spec tables (-1.4px at 48px instead of -2.4px) — its boxier counters collide at -0.05em. Pages keep `preconnect` hints to `fonts.googleapis.com` and `fonts.gstatic.com`. No other typeface is permitted on the site.

### Page inventory

- Marketing root: `index.html`, `about.html`, `changelog.html`, `contact.html`, `contact/index.html`, `privacy.html`, `status.html`
- Developer surface: `api/*.html`, `developers/index.html`, `guides/icon-font-workflow.html`, `alternatives/icomoon.html`
- Wiki: `wiki/<locale>/*.html` (16 locales × 11 pages) styled solely through `wiki/shared/wiki.css`
- `wiki/index.html` is a language redirect with no visual surface

### Hard invariants (do not break while restyling)

- `scripts/check-site-release.js` — `index.html` must keep `<span class="tag" id="versionBadge">vX.Y.Z</span>` and the `"softwareVersion"` JSON-LD value in sync with `package.json`.
- `scripts/check-ai-friendly-site.js` — `index.html` must keep the agent-mode script (`renderAgentMode`), the literal CSS rule `[data-agent-hidden] { display: none !important; }`, the `agent-notes` / `developer-resources` sections with their exact `id` + `aria-labelledby` + `data-agent-hidden` attribute strings, all `rel="alternate"` / `rel="service-desc"` head links, all JSON-LD blocks, and ≥80% meaningful `alt` text on images. `guides/icon-font-workflow.html` must keep `<article class="agent-notes" id="workflow-guide"`.
- `test/unit/website-i18n.test.js` — every `data-i18n` key in `index.html` must exist in `docs/locales/*.json`; all hreflang tags stay; `hero.title` rendering keeps `<br>` + `<em>`.
- The i18n runtime (`data-i18n` attributes + `locales/*.json` + the loader script), gtag, and the changelog fetch script stay functional.

### Adding or changing site content

1. Read this spec first, pick tokens/components from it — never invent new colors, radii, shadows, or type styles
2. Compose the page from `site.css` classes; add page-local CSS only for layout that is genuinely unique to that page, and only with `var(--*)` values
3. Headlines: sentence-case, weight 600, negative tracking (use `.t-display-*`); eyebrows and technical labels: `.t-caption-mono` / `.eyebrow`; never all-caps sans headlines, never weight 700
3a. Nav / sidebar active state: ink text (500) on a `--canvas-soft-2` pill only. Do NOT add a left-edge indicator bar on a rounded background (the `ex-app-shell-row` example above notwithstanding) — the bar bends around the radius into a bracket shape; this is a settled ruling from user review
3b. Site-wide header language: a fixed floating capsule polarity-flipped to ink glass (`rgba(23,23,23,.95)` + blur, rounded-full, Level 5 shadow + inner white ring), 60px tall, white brand/links, CTAs inverted to white pills — never a full-width bar, never a light surface. Marketing pages implement it as `.site-nav` (`--nav-h`; hero bands pull underneath via negative top margin; non-hero pages get clearance from `body:has(.site-nav)` padding). The wiki topbar (`.wiki-topbar`) mirrors the same capsule so both surfaces share one header identity. Settled rulings from user review ("沉浸式 + 悬浮"; light capsule rejected as 缺乏冲击力和对比; 透明度上调至 .95; wiki/home header 差异过大 → 统一)
3d. Every content image sits in `.media-frame`: a `--canvas-soft-2` mat with `--space-sm` padding, `--rounded-xl` outer / `--rounded-md` inner radius, Level 5 shadow + hairline rings on both layers (white-alpha variant inside `.band--dark`). Screenshots with baked-in light backdrops must never sit borderless on the page
3c. The hero mesh gradient is animated: two blob layers (`.mesh-gradient::before/::after`) drift on 26s/34s alternating transform keyframes under a static blur; disabled under `prefers-reduced-motion`. Keep animation transform-only — never animate filter or background-position
4. CTAs: black ink pill (`.btn-primary`) for the single conversion target, white pill (`.btn-secondary`) beside it; nav-scale buttons use the 6 px radius scale instead
5. The mesh gradient appears at hero scale only — never as an icon, never single-color
6. Keep the hard invariants above; run `npm run docs:check` and `npx vitest run test/unit/website-i18n.test.js` before shipping
